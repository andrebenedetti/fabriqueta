  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        className={cn(
          "fixed z-50 flex flex-col gap-4 border-border bg-card text-card-foreground shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
          side === "right" && "inset-y-0 right-0 h-full w-[min(100vw,1120px)] border-l data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right",
      ...
      >
        {children}
        {showCloseButton ? (
          <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-4 focus:ring-ring/20 disabled:pointer-events-none">
...
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
  return <div className={cn("flex flex-col gap-2 p-6", className)} data-slot="sheet-header" {...props} />;
}

function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
  return <div className={cn("mt-auto flex flex-col gap-2 p-6 sm:flex-row sm:justify-end", className)} data-slot="sheet-footer" {...props} />;
}
